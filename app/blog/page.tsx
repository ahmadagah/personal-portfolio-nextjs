...
export default function BlogIndexPage() {
  ...
  const getPostsByTopic = () => {
    ...
    const topics: Record<string, { slug: string; title: string; date: string }[]> = {};
    ...
    entries.forEach((entry) => {
      if (entry.isDirectory()) {
        ...
        const posts = fs.readdirSync(topicPath).flatMap((file) => {
          if (file.endsWith('.md')) {
            const fileContents = fs.readFileSync(path.join(topicPath, file), 'utf8');
    ...
    <section>
      {Object.entries(blogsByTopic).map(([topic, posts]) => (
        <div key={topic} className="mb-8">
          <h2 className="text-2xl font-bold capitalize">{topic.replace('_', ' ')}</h2>
          <ul>
            {posts.map((post) => (
              <li key={post.slug}>
